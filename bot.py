...
def run_repeating(config: dict | None = None) -> None:
    ...
    while not stop_requested.is_set():
        ...
        try:
            ...
            prefetched_task = None
            ux("WAITING", f"Temporary Orenya problem: {exc}. Retrying automatically.")
            stop_requested.wait(TRANSIENT_RETRY_DELAY)
            continue
        prefetched_task = None
        ...
        if not old_signature:
            if stop_requested.is_set():
                return
            ux("WAITING", "The cycle did not complete. Retrying automatically.")
            stop_requested.wait(TRANSIENT_RETRY_DELAY)
            continue
        ...
        if state == "rate_limit":
            ...
            ux("WAITING", "Orenya reported an error. Retrying automatically.")
            stop_requested.wait(TRANSIENT_RETRY_DELAY)
        else:
